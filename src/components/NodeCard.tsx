  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Governance paper: amber highlight for unconfirmed agent-inferred nodes
        isUnconfirmedAgent && 'border-l-2 border-l-amber-400/70',
      )}
    >
      {/* Hidden-from-agent badge */}
      {!node.active && (
      ...
      <div className="flex items-center gap-1.5 px-3 pt-3 pb-1.5">
        ...
        <span className={cn('text-[10px] shrink-0', node.memoryType === 'episodic' ? 'text-violet-400/70' : 'text-teal-400/70')}>
          {node.memoryType}
        </span>

        {/* Governance paper: provenance badges */}
        {node.provenance === 'agent' && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span className={cn('text-[10px] px-1.5 py-0.5 rounded border shrink-0 flex items-center gap-0.5 cursor-default',
                node.confirmed
                  ? 'bg-blue-500/10 text-blue-400/80 border-blue-500/20'
                  : 'bg-amber-500/15 text-amber-400 border-amber-500/30 animate-pulse')}>
                <Bot className="h-2.5 w-2.5" />
                {node.confirmed ? 'inferred' : 'unconfirmed'}
              </span>
            </TooltipTrigger>
            <TooltipContent>
              {node.confirmed
                ? 'Agent-inferred memory (confirmed)'
                : 'Auto-extracted by agent — confirm to accept'}
            </TooltipContent>
          </Tooltip>
        )}
        {node.provenance === 'extracted' && (
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="text-[10px] shrink-0 text-purple-400/60 cursor-default flex items-center gap-0.5">
                <Sparkles className="h-2.5 w-2.5" />extracted
              </span>
            </TooltipTrigger>
            <TooltipContent>Extracted by AI from conversation text</TooltipContent>
          </Tooltip>
        )}

        <div className="flex items-center gap-0.5 ml-auto shrink-0">
          {/* Sensitive lock — Governance paper */}
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                className={cn('h-6 w-6 flex items-center justify-center rounded transition-colors',
                  node.sensitive ? 'text-orange-400 hover:text-orange-300' : 't-muted hover:text-orange-400/70')}
                onClick={e => { e.stopPropagation(); onUpdate(node.id, { sensitive: !node.sensitive }) }}
              >
                {node.sensitive ? <Lock className="h-3.5 w-3.5" /> : <Unlock className="h-3.5 w-3.5" />}
              </button>
            </TooltipTrigger>
            <TooltipContent>{node.sensitive ? 'Sensitive — excluded from agent context. Click to unmark.' : 'Mark as sensitive (private)'}</TooltipContent>
          </Tooltip>
          ...
          <button className="h-6 w-6 flex items-center justify-center rounded t-muted hover:t-text transition-colors"
            onClick={e => { e.stopPropagation(); onEditRequest(node.id) }} title="Edit node">
            <Edit2 className="h-3.5 w-3.5" />
          </button>
          <button className="h-6 w-6 flex items-center justify-center rounded t-muted hover:text-red-400 transition-colors"
            onClick={e => { e.stopPropagation(); onDelete(node.id) }} title="Delete node">
      ...
      {isUnconfirmedAgent && (
        <div className="mx-3 mb-2 flex items-center gap-2 rounded-lg bg-amber-500/10 border border-amber-500/25 px-3 py-2">
          ...
          <p className="text-[11px] text-amber-300/90 flex-1">Agent extracted this — confirm to keep or delete.</p>
      ...
      <div className="px-3 pb-3 mt-auto">
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="flex items-center gap-2 mb-1.5 cursor-help">
              ...
              <span className="text-[10px] t-muted tabular-nums w-10 text-right">{decayLabel(decay)}</span>
            </div>
          </TooltipTrigger>
          <TooltipContent side="bottom" className="max-w-[220px]">
            <p className="font-medium mb-1">Retention: {Math.round(decay * 100)}%</p>
            <p className="text-[11px] t-muted leading-relaxed">
              Recency × 0.4 + Importance × 0.35 + Confidence × 0.25.
            </p>
          </TooltipContent>
        </Tooltip>
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {/* Xu 2025: distill fading episodic memory into a semantic fact */}
          {canDistill && (
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  onClick={e => { e.stopPropagation(); onDistill!(node) }}
              ...
              <TooltipContent>
                Distil this fading episodic memory into a lasting semantic fact (via AI)
              </TooltipContent>
            </Tooltip>
          )}
        </div>
